...
def runtest():
    ...
    parser = OptionParser(
        ...
        description="Given a file of searches and URLs (absolute, or relative to the search HTTP endpoint) that should be returned in order of importance, outputs a file indicating the 0-indexed position each URL was found on the first page, plus an 'accuracy' score; prints out the query and accuracy score as it goes. Prints a final summary including the average accuracy score. Errors in testing a query (eg HTTP failures) are treated as all expected matches being missing.",
    )
